...
enum TrafiAPI {
    case nearbyStops(latitiude: Double, longitiude: Double)
...
extension TrafiAPI: TargetType {
    ...
    var task: Task {
        switch self {
        case let .nearbyStops(latitiude, longitiude):
            return .requestParameters(parameters: ["lat": latitiude, "lng": longitiude, "api_key": apiKey], encoding: URLEncoding.queryString)
    ...
    var sampleData: Data {
        switch self {
        case .nearbyStops(let latitiude, let longitiude):
